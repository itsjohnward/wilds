...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train):
    ...
    batch_idx = 0
    iterator = tqdm(dataset['loader']) if config.get('progress_bar') else dataset['loader']
    ...
    for batch in iterator:
        ...
        if config.get('process_outputs_function') is not None:
            y_pred = process_outputs_functions[config.get('process_outputs_function')](y_pred)
        epoch_y_pred.append(y_pred)
        epoch_metadata.append(detach_and_clone(batch_results['metadata']))

        if train and (batch_idx+1) % config.get('log_every')==0:
    ...
    if config.get('scheduler_metric_split')==dataset['split']:
...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric):
    for epoch in range(epoch_offset, config.get('n_epochs')):
        ...
        curr_val_metric = val_results[config.get('val_metric')]
        general_logger.write(f'Validation {config.get("val_metric")}: {curr_val_metric:.3f}\n')

        if best_val_metric is None:
            is_best = True
        else:
            if config.get('val_metric_decreasing'):
...
def evaluate(algorithm, datasets, epoch, general_logger, config, is_best):
    ...
    for split, dataset in datasets.items():
        if (not config.get('evaluate_all_splits')) and (split not in config.get('eval_splits')):
            continue
        epoch_y_true = []
        epoch_y_pred = []
        epoch_metadata = []
...
def save_pred_if_needed(y_pred, dataset, epoch, config, is_best, force_save=False):
    if config.get('save_pred'):
        prefix = get_pred_prefix(dataset, config)
        if force_save or (config.get('save_step') is not None and (epoch + 1) % config.get('save_step') == 0):
            save_pred(y_pred, prefix + f'epoch:{epoch}_pred')
        if (not force_save) and config.get('save_last'):
            save_pred(y_pred, prefix + f'epoch:last_pred')
        if config.get('save_best') and is_best:
...
def save_model_if_needed(algorithm, dataset, epoch, config, is_best, best_val_metric):
    ...
    if config.get('save_step') is not None and (epoch + 1) % config.get('save_step') == 0:
        save_model(algorithm, epoch, best_val_metric, prefix + f'epoch:{epoch}_model.pth')
    if config.get('save_last'):
